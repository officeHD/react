...
export const getRate = (state, cb, insureAge) => {
    ...
    let socialSecFlag = state.insuredsData.insuSocialSecFlag;
    let data = {
...
export const getSureurl = (data, cb) => {
    zAJAX(`${ctx}/app/common/healthcheck`, data, cb)
}
...
export const sendData = (state, cb) => {
    ...
    let data = {
        ...
        "appnt": {
            ...
            "sex":getIdCardGender( state.appntData.sex),
        ...
        "insureds": [{
            ...
            "insuSex": getIdCardGender(state.insuredsData.insuIdNum),
            "insuImparts": [],
...
export const underwriting = (state, cb) => {
    ...
    let data = {
        ...
        "appnt": {
            ...
            "occupationCode": state.occupation.holderNo,
            "name": state.appntData.name,
            "sex": getIdCardGender(state.appntData.idNum),
            "email": state.appntData.email,
            "nationality": "CHN",
            "province": state.appntData.province,
            "city": state.appntData.city,
...
export const payOrder = (state, cb) => {
    ...
    let data = {
        ...
        "bankCode": state.payData.bankCode[0],
        "bankAccNo": state.payData.bankNum,
        "payMode": "4",
        "sumPrem": state.varietyData.fee,
        "sumAmnt": 200000,
        "renewalAccName": state.appntData.name,
        "renewalAccNo": state.payData.bankNum,
        "bankName": bankName,
...
export const checkholder = (state) => {
    var pass = true;
    if (!state.appntData.idType && !checkData('投保人身份证', state.appntData.idNum)) {
        pass = false;
        return;
    }
    if (!checkData('投保人姓名', state.appntData.name) ||
        !checkData('投保人电话', state.appntData.phone) ||
        !checkData('投保人证件号', state.appntData.idNum) ||
...
export const checkInsurant = (state) => {
    ...
    if (state.insuredsData.relationsWithCustomer == "04") {
        ...
        insuIdNum = state.appntData.idNum;
...
export const checkAge = (state) => {
    ...
    if (insurantAge >= 0) {
        ...
        //         tip = "当30天<=被保险人年龄<5周岁，最高保额30万";
        //         pass = false;
        //     } else if (insurantAge <= 40 && insurantAge >= 5) {
        ...
        //         pass = false;
        //     } else if (insurantAge <= 60 && insurantAge > 45 && state.varietyData.amnt > 30) {
    ...
    if (state.appntData.idNum && state.insuredsData.insuIdNum) {
        if (relation == "00") { //配偶
            if (getIdCardGender(state.appntData.idNum) == getIdCardGender(state.insuredsData.insuIdNum)) {
                tip = "配偶性别相同 无法投保";
                pass = false;
...
export const getIdCardGender = (val) => {
    let sex = parseInt(val.substr(16, 1)) % 2 == 1 ? 0 : 1;
    return sex;
}
//获取出生日期
