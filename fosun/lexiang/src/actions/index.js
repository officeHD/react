 ...
 * @Last Modified time: 2019-04-09 15:45:16
 */
...
export const CHANGE_INSURE_COVER = 'CHANGE_INSURE_COVER'
...
export const changeInsureCover = (val) => ({ type: CHANGE_INSURE_COVER, val })
...
//上传身份证图片
export const upLoadImg = (event, typeWay) => (dispatch, getState) => {
...
export const insertOrder = () => (dispatch, getState) => {
    ...
    api.sendData(state, msg => {
        ...
        if (msg.result.toString() === '1') {
            ...
            api.getSureurl({ orderNum: msg.orderNo }, res => {
                
                if (res.result.toString() === '1') {
                    dispatch(changeIsLoading(false))
                    let reqUrl = res.reqUrl.replace(/\+/g, '%2B');
                    console.log(reqUrl);
