...
export function getDataFromUrl() {
    ...
    AppActionCreators.initAppProduct("HXHXH001A");
    InsuranceActionCreators.initInsProduct("HXHXH001A");
...
export function getInsuranceInfo() {
    ...
    let arr = ['HXHXH001A'];
    if (stakeholder.subCode) {
        arr[arr.length] = stakeholder.subCode;
    }
    if (stakeholder.subCode1) {
        arr[arr.length] = stakeholder.subCode1;
    }
...
export function saveInsuranceInfo() {
    ...
    let cb = msg => {
        if (msg.result.toString() === '1') {
            ...
            window.location = "?type=" + type + "&workNum=" + workNum + "&productId=HXHXH001A&pid=" + msg.id + "#/share";
