...
export default class out extends Component {
    ...
    leftPage(){
      if(this.state.type){
        window.open('',"_blank").location=`${ctx}/static/pdf/anbang/suiyile/product.pdf`
      }else{
         if(window.minsheng){
            window.minsheng.OpenPdfActivity(`/static/pdf/anbang/suiyile/product.pdf`)
        }else{
              tianbai.openPdfController(`/static/pdf/anbang/suiyile/product.pdf`) 
        }
      }
    ...
    centerPage(){
       if(this.state.type){
          window.open('',"_blank").location=`${ctx}/static/pdf/anbang/suiyile/listrule.pdf`
       }else{
          if(window.minsheng){
            window.minsheng.OpenPdfActivity(`/static/pdf/anbang/suiyile/listrule.pdf`)
        } else{
           tianbai.openPdfController(`/static/pdf/anbang/suiyile/listrule.pdf`) 
        }
       }
    ...
    rightPage(){
      if(this.state.type){
         window.open('',"_blank").location=`${ctx}/static/pdf/anbang/suiyile/pricelist.pdf`
      }else{
        if(window.minsheng){
            window.minsheng.OpenPdfActivity(`/static/pdf/anbang/suiyile/pricelist.pdf`)
        }else{
            tianbai.openPdfController(`/static/pdf/anbang/suiyile/pricelist.pdf`)  
        }
      }
            ...
            <div className={style.notice}>
                <div className={style.notice_item}>
                    ...
                    <span className={style.notice_list}>
                        ...
                        <span  onClick={this.leftPage} className={style.a}>产品说明书</span> 
                        <span  onClick={this.rightPage} className={style.a}>风险保费表</span> 
                        <span  onClick={this.centerPage} className={style.a}>保险条款</span> 
                    </span>
                    
                    </div>
                   
                   <div className={style.notice_item2}></div>
