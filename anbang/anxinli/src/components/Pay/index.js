...
import supportBank from '../asset/json/supportBank.json';
...
export default class Pay extends Component {
    ...
    onInsuranceChange() {
     ...
     componentDidMount() {    
        ...
        if(!this.state.stakeholder.bbrBirthday){
            if(window.minsheng){
    ...
    showMask(){
      if(!this.state.formshow){
         if(checkData('姓名',this.state.stakeholder.cardName)
          &&checkData('账户号',this.state.stakeholder.cardNo)
          &&checkData('支行',this.state.stakeholder.bankcard.no)){
            this.setState({
                ...
                <TitleBar title="安邦安鑫利两全保险（万能型）A款" />
                 <div className={style.pay_content}>
                  ...
                  <ul className={style.content_uls}>
                        ...
                        <li className={style.list}>
                            <label>开户银行</label>
                              <Banks banks={this.state.stakeholder.bankDatas} 
                                bank={this.state.stakeholder.bankcard}
                              />
                        </li>
                        <li className={style.list}>
                            <label>开户地址</label>
                            <Place closeCity={this.state.stakeholder.closeCity} 
                              unionBankId={this.state.stakeholder.bankcard.no} 
                              cityDatas={this.state.stakeholder.cityDatas} 
                              bankprovinceDatas={this.state.stakeholder.bankprovinceDatas} 
                              province={this.state.stakeholder.cardPro} 
                              city={this.state.stakeholder.cardCity}
                            />
                        </li>
                        <li className={style.list}>
                            <label>开户支行</label>
                            <Banklist closeCity={this.state.stakeholder.closeCity} 
                              banks={this.state.stakeholder.banksData} 
                              bank={this.state.stakeholder.bankcards}  
                              unionBankId = {this.state.stakeholder.bankcard.no}  
                              provinceId={this.state.stakeholder.cardPro.no} 
                              townId={this.state.stakeholder.cardCity.no}
                           />
                        </li>  
                        <li className={style.list}>
                            <label>账户号</label>
