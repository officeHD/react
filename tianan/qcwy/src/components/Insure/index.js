...
export default class App extends Component {
    ...
    next() {
        if(checkData('投保人姓名',this.state.stakeholder.tbrName) 
            ...
            }else if(!this.state.stakeholder.sameAs){
                if(checkData('被保人姓名',this.state.stakeholder.bbrName)
                    &&checkData('被保人身份证',this.state.stakeholder.bbrNo)
                    &&checkData('被保人手机',this.state.stakeholder.bbrPhone)
                    &&checkData('被保人邮箱',this.state.stakeholder.bbrEmail)
                    &&checkData('被保人住址',this.state.stakeholder.bbrAdd)
                    
                    &&checkData('被保人邮政编码',this.state.stakeholder.bzipCode)
                    )
                {
                    if(!this.state.stakeholder.bjob.job){
                           Toast.info('请选择被保人职业',2);  
                    }else if(!this.state.stakeholder.bbrCountry.name){
                        Toast.info('请选择被保人地址',2);  
