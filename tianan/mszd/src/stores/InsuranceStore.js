...
var beginDate = year + "-" + month + "-" + day; //保险起期
var yearLong = 1; //保障年限
var isLongTimeIDs = 0; //身份证是否长期有效
var endDate = year + yearLong + "-" + month + "-" + day; //保险止期
var minBirthday = year + "-" + month + "-" + day;
    ...
    if (name === "bbrNo") {
        ...
        if (val != null && val != "") {
            if (val.length === 18) {
                ...
                if (last % 2 != 0) {
                    stakeholder.bbrSex = "01"; //男
                } else {
                    stakeholder.bbrSex = "02"; //女
                }
                ...
                if (stakeholder.bbrAge >= 3 && stakeholder.bbrAge <= 6) {
                    stakeholder.insureAge = 1;
                    changeinsurePrice()
                } else if (stakeholder.bbrAge >= 7 && stakeholder.bbrAge <= 20) {
                    stakeholder.insureAge = 2;
                    changeinsurePrice()
                }
            }
        }
...
function setBeginDate(val) {
    stakeholder.beginDate = val;
    let d = new Date(val);
    let endyear = d.getFullYear() + 1;
    let endmonth = (d.getMonth() + 1) < 10 ? '0' + (d.getMonth() + 1) : (d.getMonth() + 1);
    let endday = d.getDate() < 10 ? '0' + d.getDate() : d.getDate();
    stakeholder.endDate = endyear + "-" + endmonth + "-" + endday;
...
//更改保险类型
function changeinsureTyoe(val) {
    stakeholder.insureType = val;
    changeinsurePrice()
}
//更改年龄
function changeinsureAge(val) {
    stakeholder.insureAge = val;
    changeinsurePrice();
}

function changeinsurePrice() {
    if (stakeholder.insureAge === 1) {
        if (stakeholder.insureType === 1) {
            stakeholder.insurePrice = 180;
        } else {
            stakeholder.insurePrice = 260;
        }
    } else {
        if (stakeholder.insureType === 1) {
            stakeholder.insurePrice = 120;
        } else {
            stakeholder.insurePrice = 200;
        }
    }
}
...
function savePayResult(obj) {
    ...
    payResult.startDate = obj.message.startDate;
    payResult.staffName = obj.staffName;
    payResult.phone = obj.phone;
    payResult.endDate = obj.message.endDate;
    payResult.waterNo = obj.message.waterNo;
    payResult.policyNo = obj.message.policyNo;
    payResult.epolicyAddress = obj.policyUrl;
}
...
function handleAction(action) {
    switch (action.type) {
        ...
        case 'switchIDTimes':
            ...
            break;
        case 'setBeginDate':
            setBeginDate(action.val);
            emitChange();
            break;

        case 'changeinsureTyoe':
            changeinsureTyoe(action.val);
            emitChange();
            break;
        case 'changeinsureAge':
            changeinsureAge(action.val);
            emitChange();
            break;
